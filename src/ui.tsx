...
const App: React.FC = () => {
  ...
  const [newAnnotationDescription, setNewAnnotationDescription] =
    React.useState("");
  ...
  const [parentFrameName, setParentFrameName] = React.useState<string | null>(
    null
  );
  ...
  >(null);
  // 편집 중인 주석 상태 관리
  const [editingAnnotations, setEditingAnnotations] = React.useState<
    Record<string, { description: string }>
  >({});
  // 편집 중인 주석의 변경 상태 관리
  const [isAnnotationEdited, setIsAnnotationEdited] = React.useState<
    Record<string, boolean>
  >({});
    ...
    window.onmessage = (event) => {
      ...
      switch (message.type) {
        ...
        case "ANNOTATION_CREATED":
          ...
          // 최상위 프레임 이름 설정
          setParentFrameName(
            message.parentFrameName || message.annotation.frameName
          );
        ...
        case "ANNOTATION_DELETED":
          // 삭제된 주석을 상태에서 제거
          setAnnotations((prev) =>
            prev.filter((annotation) => annotation.id !== message.id)
          );
          break;
      }
    };
  ...
  React.useEffect(() => {
    ...
    // 모든 주석 요청도 함께 전송
    parent.postMessage({ pluginMessage: { type: "GET_ALL_ANNOTATIONS" } }, "*");
  }, []);

  // 주석이 로드되면 페이지 확장 상태 초기화
  React.useEffect(() => {
    if (annotations.length > 0) {
      // 페이지별 기본 확장 상태 설정 (모두 열림)
      const pageGroups = annotations.reduce(
        (acc, annotation) => {
          acc[annotation.pageName] = true;
          return acc;
        },
        {} as Record<string, boolean>
      );
      setExpandedPages(pageGroups);

      // 기존 프레임 확장 상태 초기화 로직
      if (!selectedFrameName) {
        const frameGroups = annotations.reduce(
          (acc, annotation) => {
  ...
  const handleDeleteAnnotation = (annotationId: string) => {
    if (
      window.confirm(
        "이 주석을 삭제하시겠습니까? 삭제 후에는 복구할 수 없습니다."
      )
    ) {
      parent.postMessage(
        {
  ...
  const handleUpdateAnnotation = (annotationId: string) => {
    ...
    if (isAnnotationChanged(annotationId)) {
      ...
      //   __html: DOMPurify.sanitize(String(editedAnnotation.description)),
      // }}
  ...
  const isAnnotationChanged = (annotationId: string) => {
    ...
    return originalAnnotation.description !== editedAnnotation.description;
  };

  // 주석 내용 변경 핸들러
  const handleAnnotationDescriptionChange = (
    annotationId: string,
              ...
              {annotations.length > 0 ? (
                <>
                  ...
                  {Object.entries(
                    annotations.reduce(
                      (acc, annotation) => {
                        if (!acc[annotation.pageName]) {
                          acc[annotation.pageName] = {};
                        }
                        if (!acc[annotation.pageName][annotation.frameName]) {
                          acc[annotation.pageName][annotation.frameName] = [];
                        }
                        acc[annotation.pageName][annotation.frameName].push(
                          annotation
                        );
                        return acc;
                      },
                    ...
                    <div key={page} className="mb-3">
                      ...
                      {/* 페이지가 확장되었을 때만 프레임 표시 */}
                      {expandedPages[page] !== false && (
                        <div className="ml-6">
                          {/* 프레임별로 그룹화 */}
                          {Object.entries(frameGroups).map(
                            ([frameName, frameAnnotations]) => (
                                  ...
                                  <span
                                    ...
                                    onClick={() => {
                                      ...
                                      if (frameAnnotations.length > 0) {
                                        handleScrollToAnnotationGroup(
                                          frameAnnotations[0].frameId
                                        );
                                      }
                                    }}
                                  >
                                    ...
                                    {frameAnnotations.map((annotation) => (
                                      <div
                                        ...
                                        onClick={() => {
                                          ...
                                          // 해당 주석의 프레임명도 선택
                                          handleSelectFrameName(
                                            annotation.frameName
                                          );
                                        }}
                                      >
                                        <div className="w-5 h-5 flex items-center justify-center">
                ...
                <>
                  ...
                  {selectedFrame && (
                    <div className="flex items-center gap-2 ml-4">
                      ...
                      >
                        {parentFrameName || selectedFrame.name}
                      </span>
                      <div className="w-3 h-3 rounded-full bg-accent ml-1 flex items-center justify-center">
                        <Check size={10} />
                      </div>
        ...
        <div className="flex-1 p-4 flex flex-col">
          {(() => {
            ...
            if (annotations.length === 0) {
              return (
                <div className="flex flex-col items-center justify-center h-full">
                  ...
                  <Button onClick={handleCreateAnnotation} className="mt-10">
                    Add Description!!!
                  </Button>
                </div>
              );
            ...
            const currentFrameName =
              selectedFrameName && frameNames.includes(selectedFrameName)
                ? selectedFrameName
                : frameNames.length > 0
                  ? frameNames[0]
                  : null;
            ...
            // 현재 표시할 프레임 정보
            const currentFrame = currentFrameName
              ? frameGroups[currentFrameName]
              : null;
                    ...
                    {currentFrame.annotations.length > 0 ? (
                      ...
                      <DragDropContext
                        onDragEnd={(result) => {
                          ...
                          // 드래그 앤 드롭 결과 처리
                          const items = [...currentFrame.annotations].sort(
                            ...
                            >
                              {[...currentFrame.annotations]
                                ...
                                .map((annotation, index) => {
                                  ...
                                  const editingData = editingAnnotations[
                                    annotation.id
                                  ] || {
                                    description: annotation.description,
                                  };
                                      ...
                                      {(provided) => (
                                        ...
                                        >
                                          ...
                                          <Editor
                                            ...
                                            onChange={(value) =>
                                              handleAnnotationDescriptionChange(
                                                annotation.id,
                                                value
                                              )
                                            }
                                            onBlur={() =>
                                              handleBlur(annotation.id)
                                            }
